...
export default function CustomerActionDropdown({
  openDropdownId,
  dropdownPosition,
  customer,
  deletingId,
  ...
  const canEdit = canPerform("customerUpdate");
  const canDelete = canPerform("customerDelete");
  ...
  if (!openDropdownId || !dropdownPosition || !customer) return null;
  ...
  return createPortal(
    ...
    >
      <button
        onClick={() => { onView(customer); onClose(); }}
