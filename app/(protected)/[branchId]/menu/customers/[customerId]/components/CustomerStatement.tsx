...
import { BranchType, CustomerType, PaymentType, SalePopulatedType } from "@/types";
...
export type CustomerStatementRef = {
  print: () => void;
};

const CustomerStatement = forwardRef<CustomerStatementRef, CustomerStatementProps>(
  function CustomerStatement({ customer, sales, payments, fullAddress, isOpen, onClose, branchPromise }, ref) {
      ...
      <>
        ...
        >
          <div style={{ padding: 24, maxWidth: 800, margin: "0 auto" }}>
            ...
            <h1 className="text-2xl font-bold mb-4">Statement — {customer.name}</h1>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16, marginBottom: 24, fontSize: 14 }}>
              <div><strong>Due:</strong> {formatCurrency(Number(customer.due ?? 0))}</div>
              <div><strong>Total Credit:</strong> {formatCurrency(Number(customer.totalCreditPurchase ?? 0))}</div>
              <div><strong>Credit Limit:</strong> {formatCurrency(Number(customer.creditLimit ?? 0))}</div>
            </div>
            <h2 style={{ fontWeight: 600, marginTop: 16, marginBottom: 8 }}>Sales</h2>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div className="relative bg-white dark:bg-neutral-900 rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-auto">
              <div className="p-6">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">{customer.name} • {fullAddress}</p>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
                  <div className="rounded-lg bg-amber-50 dark:bg-amber-900/20 p-3">
                    <p className="text-xs text-amber-600 dark:text-amber-400 font-medium">Current Due</p>
                    <p className="text-lg font-bold text-gray-900 dark:text-white">{formatCurrency(Number(customer.due ?? 0))}</p>
                  </div>
                  <div className="rounded-lg bg-blue-50 dark:bg-blue-900/20 p-3">
                    <p className="text-xs text-blue-600 dark:text-blue-400 font-medium">Total Credit</p>
                    <p className="text-lg font-bold text-gray-900 dark:text-white">{formatCurrency(Number(customer.totalCreditPurchase ?? 0))}</p>
                  </div>
                  <div className="rounded-lg bg-emerald-50 dark:bg-emerald-900/20 p-3">
                    <p className="text-xs text-emerald-600 dark:text-emerald-400 font-medium">Credit Limit</p>
                    <p className="text-lg font-bold text-gray-900 dark:text-white">{formatCurrency(Number(customer.creditLimit ?? 0))}</p>
                  </div>
                  <div className="rounded-lg bg-gray-50 dark:bg-neutral-800 p-3">
                    ...
                    <p className="text-lg font-bold text-gray-900 dark:text-white">
                      {formatCurrency(Math.max(0, Number(customer.creditLimit ?? 0) - Number(customer.due ?? 0)))}
                    </p>
                  </div>
                </div>
                <p className="text-xs text-gray-500">Showing {sales.length} sales, {payments.length} payments. Use Print for full export.</p>
