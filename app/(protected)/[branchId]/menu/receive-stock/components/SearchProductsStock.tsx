  ...
  return (
    ...
    >
      <div className="flex-1 min-w-0">
        ...
        <p className="">
          ...
          {product.details.nickname && (
            <span className="text-gray-500 ml-2">({product.details.nickname})</span>
          )}
        </p>
      ...
      <div className="text-sm text-gray-600 dark:text-gray-400 shrink-0">
        {isStockLoading || stockItem?.basePrice === undefined ? (
          <UnitSkeleton />
        ) : (
          <div className="flex items-center gap-3">
            <div className="flex flex-col items-start gap-1">
              ...
              <p className="font-medium">
                ₵{stockItem?.basePrice?.toFixed(2) ?? "—"}
              </p>
            </div>
            <div className="flex flex-col items-start gap-1">
              <p className="text-xs text-gray-500 dark:text-gray-500">Stock:</p>
              <p className="font-medium">{stockItem?.stock ?? "—"}</p>
            </div>
          </div>
        )}
      </div>
...
const SearchProductsStock = ({
  onSelectProduct,
  products: productsPromise,
  stockData: stockDataPromise,
}: SearchProductsStockProps) => {
...
export default SearchProductsStock;
