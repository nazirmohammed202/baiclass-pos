...
import { BranchType, ReceiveStockItem as ReceiveStockItemType } from "@/types";
import { ReceiveStockSettings } from "../hooks/useReceiveStockSettings";

type ReceiveStockItemProps = {
  item: ReceiveStockItemType;
  index: number;
  ...
  priceSettings: Pick<
    ReceiveStockSettings,
    "autoCalcWholesale" | "autoCalcRetail" | "roundWholesale" | "roundRetail"
  >;
};
...
const ReceiveStockItem = ({
  ...
  branchSettings,
  priceSettings,
}: ReceiveStockItemProps) => {
  const { stockMap, isStockLoading } = useStock();
  const stockItem = stockMap.get(item.product.details._id);
  const [pulsingButton, setPulsingButton] = useState<"minus" | "plus" | null>(null);
  const [isEditModalOpen, setIsEditModalOpen] = useState(false);
  const [focusField, setFocusField] = useState<
    "quantity" | "unitPrice" | "subtotal" | "discount" | "wholesalePrice" | "retailPrice" | undefined
  >(undefined);
  ...
  return (
    <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between p-2 sm:p-3 bg-white dark:bg-neutral-900 rounded-lg border border-gray-200 dark:border-neutral-800 hover:shadow-sm transition-shadow gap-2 sm:gap-0">
      ...
      <div className="flex items-center justify-between sm:justify-end gap-2 sm:gap-4 lg:gap-6 shrink-0 order-2 flex-wrap sm:flex-nowrap">
        ...
        {/* Discount Badge - Clickable */}
        {discount > 0 && (
          <button
            onClick={() => {
              setFocusField("discount");
              setIsEditModalOpen(true);
            }}
            className="px-2 py-1 text-xs font-semibold bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 rounded-full hover:bg-green-200 dark:hover:bg-green-900/50 transition-colors"
