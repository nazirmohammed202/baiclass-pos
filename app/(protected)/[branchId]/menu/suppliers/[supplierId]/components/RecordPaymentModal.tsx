...
export default function RecordPaymentModal({
  ...
  const buildPayload = (): RecordSupplierPaymentPayload | null => {
  ...
  const handleSubmit = async (shouldPrint: boolean) => {
    ...
    setSaving(true);
    const result = await recordSupplierPayment(supplierId, branchId, payload);
    setSaving(false);
    if (result.success) {
      if (shouldPrint) {
  ...
  const printReceipt = (payload: RecordSupplierPaymentPayload) => {
    ...
    const content = `
      <!DOCTYPE html>
      <html>
        <head><title>Payment Receipt - ${supplierName}</title></head>
        <body style="font-family: system-ui, sans-serif; padding: 24px; max-width: 400px; margin: 0 auto;">
          ...
          <p style="margin: 0 0 8px; font-size: 14px;"><strong>Supplier:</strong> ${supplierName}</p>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto py-4">
      ...
      <div className="relative bg-white dark:bg-neutral-900 rounded-lg shadow-xl max-w-md w-full mx-4 p-6 max-h-[90vh] overflow-y-auto">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
          {supplierName} • Due: <span className="font-medium text-gray-900 dark:text-white">{formatCurrency(dueBalance)}</span>
        </p>
        <form
          onSubmit={(e) => {
            e.preventDefault();
